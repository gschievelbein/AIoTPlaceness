...
class MultiDEC(nn.Module):
    ...
    def fit_predict_ssldec(self, full_dataset, train_dataset, test_dataset, args, CONFIG, lr=0.001, batch_size=256, num_epochs=10, update_time=1, save_path=None, tol=1e-3, kappa=0.1):
        full_num = len(full_dataset)
        full_num_batch = int(math.ceil(1.0 * len(full_dataset) / batch_size))
        train_num = len(train_dataset)
        train_num_batch = int(math.ceil(1.0 * len(train_dataset) / batch_size))
    ...
    def fit_predict_transductive(self, full_dataset, train_dataset, args, CONFIG, lr=0.001, batch_size=256, num_epochs=10, update_time=1, save_path=None, tol=1e-3, kappa=0.1):
        full_num = len(full_dataset)
        full_num_batch = int(math.ceil(1.0 * len(full_dataset) / batch_size))
        train_num = len(train_dataset)
        train_num_batch = int(math.ceil(1.0 * len(train_dataset) / batch_size))
        ...
        full_short_codes = full_dataset[:][0]
        train_short_codes = train_dataset[:][0]
        full_labels = full_dataset[:][3].squeeze(dim=0).data.cpu().numpy()
        train_labels = train_dataset[:][3].squeeze(dim=0).data.cpu().numpy()
        df_full = pd.DataFrame(data=full_labels, index=full_short_codes, columns=['label'])
        df_train = pd.DataFrame(data=train_labels, index=train_short_codes, columns=['label'])
        _, image_ind = align_cluster(train_labels, train_image_pred)
        _, text_ind = align_cluster(train_labels, train_text_pred)
        ...
        p, _, _ = self.target_distribution(q, r)
        initial_pred = torch.argmax(p, dim=1).numpy()
        initial_acc = accuracy_score(full_labels, initial_pred)
        initial_nmi = normalized_mutual_info_score(full_labels, initial_pred, average_method='geometric')
        initial_f_1 = f1_score(full_labels, initial_pred, average='macro')
        print("#Initial measure: acc: %.4f, nmi: %.4f, f_1: %.4f" % (initial_acc, initial_nmi, initial_f_1))
        ...
        for epoch in range(num_epochs):
            ...
            test_unsupervised_image_loss /= full_num
            test_unsupervised_text_loss /= full_num
            test_pred = torch.argmax(test_p, dim=1).numpy()
            test_acc = accuracy_score(full_labels, test_pred)
            test_nmi = normalized_mutual_info_score(full_labels, test_pred, average_method='geometric')
            test_f_1 = f1_score(full_labels, test_pred, average='macro')
            print("#Test measure %3d: acc: %.4f, nmi: %.4f, f_1: %.4f" % (
                epoch + 1, test_acc, test_nmi, test_f_1))
            print("#Test loss %3d: unsup img: %.4f, unsup txt: %.4f, super img: %.4f, super txt: %.4f" % (
                epoch + 1, test_unsupervised_image_loss, test_unsupervised_text_loss, test_supervised_image_loss, test_supervised_text_loss))
    ...
    def fit_predict_transductive_ssldec(self, full_dataset, train_dataset, args, CONFIG, lr=0.001, batch_size=256, num_epochs=10, update_time=1, save_path=None, tol=1e-3, kappa=0.1):
        full_num = len(full_dataset)
        full_num_batch = int(math.ceil(1.0 * len(full_dataset) / batch_size))
        train_num = len(train_dataset)
        train_num_batch = int(math.ceil(1.0 * len(train_dataset) / batch_size))
